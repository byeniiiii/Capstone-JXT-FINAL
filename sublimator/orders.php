...
// Check if the user is logged in and is a sublimator
if (!isset($_SESSION['sublimator_id'])) {
    header("Location: ../index.php");
    exit();
}

// Fetch only sublimation orders assigned to this sublimator
$query = "SELECT 
            o.order_id, 
          ...
          WHERE o.order_type = 'sublimation' 
            AND s.sublimator_id = " . $_SESSION['sublimator_id'] . "
            AND o.order_status IN ('forward_to_sublimator', 'in_process', 'completed_by_sublimator')";

// Fetch only sublimation orders that are forwarded to the logged-in sublimator
$query = "SELECT 
            o.order_id, 
            CONCAT(c.first_name, ' ', c.last_name) AS customer_name, 
            o.created_at AS order_date,  
          ...
          WHERE o.order_type = 'sublimation' 
            AND s.sublimator_id = " . $_SESSION['user_id'] . "
            AND o.order_status IN ('forward_to_sublimator', 'in_process', 'completed_by_sublimator')";
...
<body id="page-top">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800" style="font-weight: bold; text-shadow: 1px 1px 2px #fff;">
                            Manage Sublimation Orders (Downpayment Paid)
                        </h1>
                    </div>
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#markCompleteModal" data-order-id="<?php echo $row['order_id']; ?>">Mark as Completed</button>
                                            </td>
                                        </tr>
                                <?php }
                                } else {
                                    echo "<tr><td colspan='7' class='text-center'>No sublimation orders with downpayment paid found</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="markCompleteModalLabel">Mark Order as Completed</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="mark_order_complete.php">
    ...
    <script>
        ...
        markCompleteModal.addEventListener('show.bs.modal', function (event) {
